[![Gitter](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/majidpal/MDOP)

# The DevOps Platform: Overview

The DevOps Platform is a tools environment for continuously testing, releasing and maintaining applications. Reference code, delivery pipelines, automated testing and environments can be loaded in via the concept of [Cartridges](https://github.com/majidpal/mdop-cartridge-skeleton).
...
![HomePage](https://raw.githubusercontent.com/accenture/mdop-docker-compose/master/img/home.png)
...
![HomePage](https://raw.githubusercontent.com/majidpal/mdop-docker-compose/master/img/exampleproj.png)

Once you have explored this the next step is to create your own Workspace and Project and then load another cartridge using a 'Load Cartridge' job in the 'Cartridge Management' folder (that automatically gets created in any Project).  The cartridge [development cartridge](https://github.com/accenture/mdop-cartridge-cartridge-dev/) also helps create your own cartridges.

# Quickstart Instructions

These instructions will spin up an instance in a single server in AWS ([for **_evaluation_** purposes](https://accenture.github.io/mdop-docker-compose/docs/evaluation-mode)).  Please check the [prerequisites](https://accenture.github.io/mdop-docker-compose/docs/prerequisites/).
...
1. Clone this repository and then in a terminal window (this has been tested in GitBash):
    ...
    - For example (if you don't have ~/.aws set up):
        ...
        ```./quickstart.sh -t aws -m mdop1 -a AAA -s BBB -c vpc-123abc -r eu-west-1 -u user.name -p userPassword```
        - N.B. If you see an error saying that docker-machine cannot find an associated subnet in a zone, go back to the VPC Dashboard on AWS and check the availablity zone for the subnet you've created. Then rerun the startup script and use the -z option to specify the zone for your subnet, e.g. for a zone of eu-west-1c the above command becomes:
            ...
            ```./quickstart.sh -t aws -m mdop1 -a AAA -s BBB -c vpc-123abc -r eu-west-1 -u user.name -p userPassword -z c```
...
- Run: source env.config.sh
    ...
    - **When creating a new instance of MDOP, you must delete platform.secrets.sh and regenerate it using credentials.generate.sh, else your old environment variables will get sourced as opposed to the new ones.**
...
* RUN : source env.config.sh
* RUN : ./mdop compose gen-certs ${DOCKER\_CLIENT\_CERT\_PATH}
...
Kibana 4 does not provide a configuration property that allow to define the default index pattern so the following manual procedure should be mdopted in order to define an index pattern:
...
## Documentation
Documentation can be found [on our GitHub Pages site](https://accenture.github.io/mdop-docker-compose).

## Issues
If you have any problems with or questions about this project, please contact us through [Gitter](https://gitter.im/majidpal/MDOP) or a [GitHub issue](https://github.com/majidpal/mdop-docker-compose/issues).

## Contribute
You are invited to contribute new features, fixes, or updates, large or small; we are always thrilled to receive pull requests, and do our best to process them as fast as we can. You can find more information in our [documentation](https://accenture.github.io/mdop-docker-compose/docs/contributing/).

Before you start to code, we recommend discussing your plans through a [GitHub issue](https://github.com/majidpal/mdop-docker-compose/issues), especially for more ambitious contributions. This gives other contributors a chance to point you in the right direction, give you feedback on your design, and help you find out if someone else is working on the same thing.

## Roadmap
We use this working [Roadmap](https://github.com/majidpal/mdop-docker-compose/wiki/Roadmap) to evolve and summarise plans for future features and the merge of existing PRs.

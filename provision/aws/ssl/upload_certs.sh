    ...
    echo "
	  ...
	  Examples:
	  	1) Generate and Upload Self Signed certificate.
			$0 -n mdop-selfsigned-cert -k file://selfsigned/mdop-cert-key.pem -c file://selfsigned/mdop-cert.pem 
		2) Upload custom certificate.
			$0 -n mdop-selfsigned-cert -k file://custom/mdop-cert-key.pem -c file://custom/mdop-cert.pem -e file://custom/ca-cert.pem
    "
    exit 1	
}

export SELF_SIGNED_CERT=1
export CERT_KEY='file://selfsigned/mdop-cert-key.pem'
export CERT='file://selfsigned/mdop-cert.pem'
export CERT_NAME='mdop-selfsigned-cert'
